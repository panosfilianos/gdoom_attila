...
class GDoomEnv(gym.Env, i, state_size, action_size, trainer, model_path):
    ...
    def step(self, action):
        ...
        if action >= 0:
            ...
            reward = self.get_custom_reward(self.game.make_action(a_t.tolist()))
        elif action == -1:
            # get reward from executing action
            reward = self.get_custom_reward(self.game.make_action(  np.zeros([self.action_space.n]).tolist()   ))##this and following if are equal..no sense to differentiate the mode
        elif self.mode == CPU:
...
def train(env, max_episodes, gamma, sess, coord, saver):
    ...
    while running:
        ...
        if env_done:
            ...
            action, a_dist, v, rnn_state, action_index = env.agent.get_policy_action(obs)#keys_to_action.get(tuple(sorted(pressed_keys)), 1)
