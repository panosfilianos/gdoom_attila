...
def train(env, max_episodes, gamma, sess, coord, saver, transpose=True, fps=30, zoom=None, callback=None, keys_to_action=None):
    ...
    while (not coord.should_stop()) and (env.agent.episode_count < max_episodes):
        ...
        for index in range(4):
            env.frame_stack.append(obs)
        env.stack_obs = gdoom_utils.process_frame_stack(env.frame_stack)
        ...
        while(not(ep_done)):
            ...
            prev_obs = obs
            prev_stack_obs = env.stack_obs
            obs, rew, ep_done, info = env.step(action_index)

            obs = gdoom_utils.process_frame(obs, crop, resize)
            env.frame_stack.append(obs)
            del env.frame_stack[0]
            env.stack_obs = gdoom_utils.process_frame_stack(env.frame_stack)
...
def after_step_callback(env, prev_stack_obs, stack_obs, action, reward, ep_done, v, sess, max_episodes, gamma, rnn_state):
    # Append step to buffer
    env.agent.local_AC.episode_buffer.append([stack_obs, action, reward, prev_stack_obs, False, v[0, 0]])
